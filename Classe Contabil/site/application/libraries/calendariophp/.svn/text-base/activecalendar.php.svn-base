...
// parametros adicionais para o link
public $parametros;
/*
----------------------
...
********************************************************************************
*/
function activeCalendar($param){
	$parametros = $param['parametros'];
	$year = $param['year']; //false
	$month = $param['month']; //false
	$day = $param['day']; //false
	$GMTDiff = $param['GMTDiff']; //"none"
	
    // Trata os parï¿½metros recebidos
...
function mkMonthTitle(){
	if (!$this->monthNav){
	    ...
	    $out = "<tr><td colspan='10' style='padding: 0px;'>";
	    
	    // Left Navigation
	    $out .= "<span class='navigationleft'>";
	    if ($this->actmonth==1) $out .= $this->mkUrl($this->actyear-1,"12");
	    else $out .= $this->mkUrl($this->actyear,$this->actmonth-1);
		$out .= $this->monthNavBack."</a></span>";
	    
	    // Center (month)
	    $out .= "<span class=\"".$this->cssMonthTitle."\">";
	    $out .= $this->getMonthName().$this->monthYearDivider.$this->actyear;
	    $out .= "</span>";
	    
	    // Right Navigation
	    $out .= "<span class='navigationright'>";
	    if ($this->actmonth==12) $out .= $this->mkUrl($this->actyear+1,"1");
		else $out .= $this->mkUrl($this->actyear,$this->actmonth+1);
	    $out.=$this->monthNavForw."</a></span>";
...
$yearNavLink="<a href=\"".$this->urlNav.$this->yearID.":".$year."\">";
$monthNavLink="<a href=\"".$this->urlNav.$this->yearID.":".$year."/".$this->monthID.":".$month.$this->parametros."\">";
$dayLink="<a href=\"".$this->url.$this->yearID.":".$year."/".$this->monthID.":".$month."/".$this->dayID.":".$day.$this->parametros."\">".$day."</a>";
