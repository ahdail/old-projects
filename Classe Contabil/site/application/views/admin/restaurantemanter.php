...
function alternaDivAutor(valor) {
	if (valor == -1) {
		$('#divAutor').show();
		$('#nome').focus();
	} else {
...
<?php }  ?>
<?php if ($msg == "ok"){?>
<div class="msgOk">Cadastro realizado com sucesso!</div>
<?php }?>
<form method="post" action="<?= base_url() ?>admin/artigo/manter">
<input type="hidden" name="id" value="<?=$row['id']?>">
<input type="hidden" name="idUsuario" value="<?=$row['idUsuarios']?>">
		<h1>CADASTRO DE ARTIGO</h1>
		<ul>
			<li>
				<label>Autor <span>*</span></label>
				<select name="idAutor" onchange="alternaDivAutor(this.value)" style="width: 500px">
					<option>Selecione:</option>
					<option>-------------------------------</option>
					<option value="-1">[Novo Autor]</option>
					<option>-------------------------------</option>
					<?php 
						foreach ($autor as $autor){
						if ($autor['id'] == $row['idAutores']){
				...
				<div id='divAutor' style="padding-left: 10px; display:none;">
					<ul>
						<li>
							<label>Nome: <span>*</span></label>
							<input type="text" class="campoGd" name="nome" id="nome" value="<?=$row['nome']?>" />
						</li>
						<li>
							<label>Email: <span>*</span></label>
							<input type="text" class="campoGd" name="email" id="email" value="<?=$row['email']?>" />
						</li>
						<li>
							<label>Curriculo: <span>*</span></label>
							<textarea id="curriculo" name="curriculo" class="campo" style="width: 95%"></textarea>
						</li>
					</ul>
				</div>
			</li>
			...
			<li>
				...
				<input type="checkbox" id="dtk" name="bloqueado" value="1" <? if($row['bloqueado']) echo "checked"; ?>>
			</li>
			...
			<li>
				...
				<?php 
					...
					if ($row['tipo'] == "J") {
						$marcadoTipoJuizo = "checked=checked";
					} else {
						$marcadoTipoJuizo = "";
					}
					
					if ($row['tipo'] == "D") {
						$marcadoTipoDireito = "checked=checked";
					} else {
						$marcadoTipoDireito = "";
					}
				?>
				...
				<label>&nbsp;</label><input type="checkbox" id="dtk" name="exibirDestaque" onclick="mostraEscondeCampos();" value="S" <?=$marcadoExibirDestaque ?> ><span style="font-size:12px;"> Exibir como Destaque</span></br>
				<!-- <label>&nbsp;</label><input type="checkbox" id="dtk" name="tipo" value="J"  <?=$marcadoTipoJuizo ?> ><span style="font-size:12px;"> Juízo Semanal</span></br>-->
				<label>&nbsp;</label><input type="checkbox" id="dtk" name="tipo"  value="D" <?=$marcadoTipoDireito ?> ><span style="font-size:12px;"> Temas de Direito Empresarial</span></br>
			</li>
			<li>
				<label>&nbsp;</label> 
