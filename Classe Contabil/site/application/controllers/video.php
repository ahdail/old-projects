...
class Video extends MY_Controller
{
	...
	function ver($id)
	{
    	$this->VideoModel->contador($id);
    	
		$var['rowVideo'] = $this->VideoModel->ver($id);
		$var['comentario'] = Video::exibirFormComentario($id);
		$var['indicacao'] = Video::exibirFormIndicacao($id);
		$var['exibirComentarios'] = $this->VideoModel->exibirComentarios($id);
		 
		$this->render('video', $var);
	}
	
	function listar($start = 0)
	{
	    $config = array(
    		'base_url' 		=> site_url('/video/listar/'),
    		'per_page' 		=> 10,
        ...
        $var['videos']= $this->VideoModel->exibir($start, $config['per_page']);
		$this->render('videos', $var);
	}
	...
	function rss()
	{
		...
		$videos = $this->VideoModel->exibir(0, 100);
		
		// Cria os $videos no XML
		foreach($videos as $row) {
			$item = array(
				"title" => $row['titulo'],
				"link" => base_url()."video/ver/".$row['id'],
				"description" => $row['resumo'],
			);
			
			$arquivo = array(
				array("type"=>"video/flv", "url"=>base_url()."site/videos/{$row['arquivo']}")
			);
...
//functions responsáveis pelo envio do formulário de COMENTÁRIOS do Vídeo via ajax 
	...
	function comentar($id)
	{
		...
		if ($this->form_validation->run() == FALSE){
			...
			$this->VideoModel->comentarVideo($dados);
	...
	function indicacao($id)
	{
    	...
    	if ($this->form_validation->run() == FALSE){
			...
			$mensagem = "
				...
				Leia: <a href=\"http://www.classecontabil.com.br/v3/video/ver/$id\">clique aqui</a><br><br>
				Comentario:
				<br>----------------<br>
				 {$dados['msg']}
				<br>----------------<br>
