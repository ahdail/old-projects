...
if ($acao == "excluirArquivo" && $arquivo) {
	unlink("$dir/$arquivo");
}
if ($acao == "excluirSecao" && $pasta) {
	rmdir("$pasta");
}
			...
			<td>
				...
				function excluirImagem(id) {
					if (confirm('Tem certeza que deseja excluir a imagem?')) {
						window.location = 'select_image.php?dir=<?=$dir?>&acao=excluirArquivo&arquivo='+id;
					}
				}
				function excluirSecao(id) {
					if (confirm('Tem certeza que deseja excluir a Seção?')) {
						window.location = 'select_image.php?dir=<?=$dir?>&acao=excluirSecao&pasta='+id;
					}
				}
				</script>	
				  <?php
					...
					?>
					<a href="<?php echo $_SERVER['PHP_SELF'].'?dir='.urlencode($leadon.$dirs[$i]); ?>"><img src="images/folder.png" alt="<?php echo $dir; ?>" border="0" /> <strong><?php echo $dir; ?></strong></a> <a href="javascript:excluirSecao('<?=$leadon.$dirs[$i]?>');"> [x]</a><br>
					<?php
						if($class=='b') $class='w';
						else $class = 'b';	
					}
					...
					?>
					<a href="javascript:excluirImagem('<?=$filename?>');">[x] Excluir</a><br>
					<a href="javascript:void(0)" onclick="selectImage('<?php echo $imagebaseurl.$filedir.$filename; ?>');">
						<!--<img src="images/<?php echo $icon; ?>" alt="<?php echo $files[$i]; ?>" border="0" /> <strong><?php echo $filename; ?></strong>-->
						<img src="<?php echo "".$imagebaseurl.$filedir.$filename; ?>" border=0>
						<br>
					</a><br>
					<?php
							if($class=='b') $class='w';
