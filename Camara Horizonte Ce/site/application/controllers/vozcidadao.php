...
class VozCidadao extends Controller {
	...
	function enviar() 
	{
		...
		if ($this->form_validation->run () == FALSE) {
			...
			$vozcidadao = array(
				'nome' 	=> $this->input->post('nomeremetente'),
				'email' => $this->input->post('emailremetente'),
				'id_assunto' => $this->input->post('assuntoremetente'),
				'mensagem' => $this->input->post('mensagemremetente'),
				'mostrar' 	=> "N"
			);
	...
	{
		$config = array(
    		'base_url' 		=> site_url('/vozcidadao/listar/'),
    		'per_page' 		=> 10,
		...
		$vozcidadao['comissoes'] = $this->ComissaoModel->comissaoSite();
		$vozcidadao['apoio'] = $this->ApoioModel->apoioSite();		
		$vozcidadao['legislacoes'] = $this->LegislacaoModel->legislacao();
		$vozcidadao['leismunicipais'] = $this->LegislacaoModel->leimunicipal();
		
        $this->load->view('vozcidadao',$vozcidadao);
	}
